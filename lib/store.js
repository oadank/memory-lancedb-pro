...
class MemoryDB {
  ...
  async addKBEntry(text, vector, importance = 2, category = "lesson", metadata = {}) {
    ...
    const now = Date.now();
    const id = randomUUID();
    // 按 knowledge_base 表 schema 字段顺序写入
    await kbTable.add([{
      id, text, vector, category, createdAt: now, source: metadata.source || 'auto-promote', importance,
      promotedFrom: metadata.promotedFrom || '',
      repeatCount: metadata.repeatCount || 0,
      originalCreatedAt: metadata.originalCreatedAt || now
    }]);
    return id;
  }
